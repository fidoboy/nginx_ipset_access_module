...
static void ngx_ipset_cache_initializer() {
    ...
    int rc = pthread_key_create(&ngx_ipset_cache_key,
                            ngx_destroy_ipset_session);

    if (rc != 0) {
        ngx_ipset_cache_initialize_result = rc;
    }
}
...
static int ngx_str_array_copy(ngx_pool_t* pool, ngx_array_t* dst, ngx_array_t const* src, ngx_uint_t si) {
    ...
    ngx_uint_t n = src->nelts - si;

    dst_values = ngx_array_push_n(dst, n);
    if (!dst_values) {
        return ENOMEM;
    }

    /* ngx_array_push_n() grows the array with ngx_palloc(), NOT ngx_pcalloc(),
     * so the newly pushed ngx_str_t slots hold whatever bytes happened to be
     * in that pool memory (uninitialized/garbage), not {0, NULL}. ngx_str_copy()
     * inspects dst->len and dst->data to decide whether it can reuse an
     * existing buffer, so without this memzero it can read a garbage dst->len
     * as "big enough" and memcpy() into a garbage dst->data pointer, or pass
     * a garbage pointer to ngx_pfree(). Zero the slots first so ngx_str_copy()
     * always takes the "allocate a fresh buffer" path for them. */
    ngx_memzero(dst_values, n * sizeof(ngx_str_t));
...
static char* ngx_ipset_access_server_conf_parse(ngx_conf_t* cf, ngx_command_t* command, void* pv_conf) {
    ...
    if (args[1].len == 3 && memcmp(args[1].data, "off", 3) == 0) {
        if (conf->mode != e_mode_not_configured && conf->mode != e_mode_off) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                "\"%V off\" conflicts with a previous \"blacklist\"/\"whitelist\" directive "
                "in the same context; only one of \"blacklist\", \"whitelist\", or \"off\" "
                "may be used per server block", &args[0]);
    ...
    {
        ngx_uint_t new_mode = args[0].data[0] == 'b' ? e_mode_blacklist : e_mode_whitelist;

        /* "blacklist" and "whitelist" (and "off") all write into the same
         * server-config struct (mode + sets). Without this check, using both
         * directives in the same context would silently concatenate their
         * ipsets into one list and let whichever directive appears last
         * decide the mode for ALL of them -- e.g. a "whitelist" set could
         * end up being enforced as a blacklist, or vice versa. Refuse to
         * start instead. Repeating the SAME directive multiple times to add
         * more sets is still allowed. */
        if (conf->mode != e_mode_not_configured && conf->mode != new_mode) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                "\"%V\" directive conflicts with a previous \"%s\" directive in the same "
                "context; \"blacklist\" and \"whitelist\" cannot both be used for the same "
                "server block", &args[0],
                conf->mode == e_mode_blacklist ? "blacklist" : "whitelist");
            return (char*)NGX_CONF_ERROR;
        }
    }
...
static ngx_int_t ngx_ipset_access_http_access_handler(ngx_http_request_t* request) {
    ...
    if ((conf->mode == e_mode_whitelist || conf->mode == e_mode_blacklist) &&
        ...
        u_char ip[NGX_INET6_ADDRSTRLEN + 1];
        size_t ip_len;
        ngx_ipset_session_t* session;
        ...
        ip_len = ngx_sock_ntop(request->connection->sockaddr,
                                request->connection->socklen,
                                ip, NGX_INET6_ADDRSTRLEN, 0);
        ip[ip_len] = '\0';
        ...
        if (!session) {
             ...
             * depend on set ORDER whenever IPv4 and IPv6 ipsets are mixed
             * in the same directive (a v4 address always reads as
             * IPS_TEST_INVALID_IP against an inet6-only set, and vice
             * versa). */
            result = IPS_TEST_IS_NOT_IN_SET;

            for (i = 0; i < conf->sets.nelts; i++, set++) {
                ngx_ipset_test_result_t r = ngx_test_ip_is_in_set(session, (char*)set->data, (char*)ip);
                #ifdef NGX_DEBUG
                ngx_log_debug3(NGX_LOG_DEBUG, request->connection->log, 0, "test %s %s -> %d", set->data, ip, r);
                #endif
                ...
                if (r == IPS_TEST_FAIL || r == IPS_TEST_INVALID_SETNAME) {
                    ngx_log_error(NGX_LOG_WARN, request->connection->log, 0, "Failed to test presence of IP in IPSET.");
                    result = r;
                    break;
                }

                /* IPS_TEST_IS_NOT_IN_SET, or IPS_TEST_INVALID_IP because this
                 * set's address family (inet/inet6) doesn't match the
                 * client's: neither means "found", so keep checking the
                 * remaining sets and leave `result` as IPS_TEST_IS_NOT_IN_SET. */
            }
        }

        /* whitelist: allow only IPs found in one of the configured sets,
         * block everything else (not found, or a genuine test error).
